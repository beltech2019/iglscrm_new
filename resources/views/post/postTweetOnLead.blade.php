...
<div class="container-fluid">
    <div class="div_container">
        <div class="bgwhite2 widthsmall">
			...
			@if($postData && $postData != "")
				<form method="POST" action="/replyTweetId/{{$postData->getTweet_id ? $postData->getTweet_id:'lead'}}" enctype="multipart/form-data" onsubmit="showLoader()">
			@endif
                    ...
                    <div class="heading_two ">
                        <h2><i class="bi bi-ticket iconsbg2"></i>Lead Reply </h2>
                    </div>
                </div>
           ...
           <div class="formscoulam">
            <div class="row">
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control readonly" id="lead_source" name="source" placeholder="Source" value="{{$postData?$postData->lead_source:''}}" readonly>
                        <!-- <select class="form-control readonly" placeholder="Source" id="lead_source" name="source" readonly>
                            @foreach(getSource() as $key => $getSources)
                                <option value="{{$getSources->value}}" {{$postData && $postData->lead_source==$getSources->value?'selected':''}}>{{$getSources->value}}</option>
                            @endforeach
                            <option value="Other" {{$postData && $postData->lead_source=='Other'?'selected':''}}>Other</option>
                            <option value="Portal"{{$postData && $postData->lead_source=='Portal'?'selected':''}}>Portal</option>
                            <option value="Call"{{$postData && $postData->lead_source=='Call'?'selected':''}}>Call</option>
                            <option value="Inbounced Email"{{$postData && $postData->lead_source=='Inbounced Email'?'selected':''}}>Inbounced Email</option>
                            </select>               -->
                              </div>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="mobile_no" name="mobile_no" placeholder="Mobile Number" value="{{$postData?$postData->mobile:''}}" />
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        <label for="postDate" class="form-label">Status</label>
                        <input type="text" class="form-control" id="postDate" name="postDate" placeholder="Post Date" value="{{$postData?$postData->status:''}}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                        <label for="socialUser" class="form-label">Name</label>
                        <div style="clear:both;"></div>
                        <input type="text" class="form-control" name="socialUser" id="socialUser" placeholder="Social User" value="{{$postData?$postData->first_name :''}}">
                </div>
            </div>
            ...
            <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control {{ addUIComponent('LEAD_REPLY_TEMPLATE') }}" id="postMessage" name="postMessage" rows="3" placeholder="Post Message"></textarea>
                    </div>
                    @if($postData && $postData->lead_source=="Twitter")
  ...
  @foreach($reply as $replys)
   ...
   @if(count($dmData)>0)
    ...
    <td>{!! getUrlinString($postData?$postData->description:'')!!}</td>
      <td>Text</td>
      <td></td>
